...
const args = JSON.parse(process.env.PTY_ARGS || '[]');
...
// Write a string directly to the terminal (as if the PTY had produced it)
function writeToTerminal(text) {
  const b64 = Buffer.from(text, 'utf8').toString('base64');
  process.stdout.write('DATA:' + b64 + '\n');
}

// Surface errors as visible terminal text so they aren't silently swallowed
process.on('uncaughtException', err => {
  writeToTerminal(`\r\n\x1B[31;1m[pty-proxy error] ${err.message}\x1B[0m\r\n`);
  process.exit(1);
});
...
try {
  ...
  writeToTerminal(`\r\n\x1B[31;1m[pty-proxy] Failed to spawn '${cmd}': ${err.message}\x1B[0m\r\n`);
  process.exit(1);
}
...
rl.on('line', line => {
  if (line.startsWith('INPUT:')) {
    ...
    ptyProcess.write(bytes.toString('utf8'));  // utf8, not binary — handles non-ASCII input
